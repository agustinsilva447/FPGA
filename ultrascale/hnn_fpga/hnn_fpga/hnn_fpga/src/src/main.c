...
int main()
{
	...
	int it_max = 50, source = 1, destin = 6, flag, it, x, i, y, j;
	...
	*/
	...
	if (!goHnn_fpga_cfg){
		printf("\nError cargando la configuracion de goHnn_fpga.");
	} else {
		printf("\nHnn_fpga configurado correctamente.");
	}
	status = XHnn_fpga_CfgInitialize(&goHnn_fpga, goHnn_fpga_cfg);
	if (status != XST_SUCCESS){
		printf("\nError inicializando goHnn_fpga.");
	} else {
		printf("\nHnn_fpga inicializado correctamente.");
	}
