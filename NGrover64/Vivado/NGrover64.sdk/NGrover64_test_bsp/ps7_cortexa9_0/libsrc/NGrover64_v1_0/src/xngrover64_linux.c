...
typedef struct {
    ...
    XNgrover64_uio_map maps[ MAX_UIO_MAPS ];
} XNgrover64_uio_info;

/***************** Variable Definitions **************************************/
static XNgrover64_uio_info uio_info;
...
static int uio_info_read_name(XNgrover64_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XNgrover64_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XNgrover64_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XNgrover64_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XNgrover64_Initialize(XNgrover64 *InstancePtr, const char* InstanceName) {
	XNgrover64_uio_info *InfoPtr = &uio_info;
