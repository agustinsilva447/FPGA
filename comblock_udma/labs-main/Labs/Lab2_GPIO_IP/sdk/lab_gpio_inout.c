...
int main(void)
{
	 ...
	 *
    */
	Status = <???>(&pmoda_o, XPAR_PMOD_A_O_DEVICE_ID);
	 ...
	 *
    */
	PSConfigPtr = XGpioPs_LookupConfig(<???>);
	if (PSConfigPtr == NULL)
		return XST_FAILURE;
	...
	xil_printf("GPIO PS Initialization ok\r\n");
	...
	for (int i = 0; i < 256; i++){
		...
		XGpio_DiscreteWrite(&<???>, 1, i); //Single-channel GPIO (PMODA_GPIO1)

		/*Discrete read pmodb*/
		readVal = XGpio_DiscreteRead(&<???>, 1); //Single-channel GPIO (PMODB_GPIO1)
